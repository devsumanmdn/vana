...
import PlayNextIcon from "@mui/icons-material/SkipNextRounded";
import PlayPreviousIcon from "@mui/icons-material/SkipPreviousRounded";
...
const useStyle = makeStyles({
  ...
  backgroundContainer: {
    position: "fixed",
    zIndex: -999999,
    top: ({ transparentMode }: settings) => (transparentMode ? 30 : 0),
    left: ({ transparentMode }: settings) => (transparentMode ? 30 : 0),
    height: ({ transparentMode }: settings) =>
      transparentMode ? "calc(100% - 60px)" : "100%",
    width: ({ transparentMode }: settings) => {
      console.log(transparentMode);
      return transparentMode ? "calc(100% - 60px)" : "100%";
    },
    backgroundColor: ({ transparentMode }: settings) =>
      transparentMode ? "transparent" : "#000",
  },
  background: {
    height: "100%",
    width: "100%",
    ...
    filter: `blur(1.20rem) brightness(0.5)`,
    opacity: ({ transparencyAmount }: settings) => transparencyAmount / 100,
...
const Player = ({
  ...
  useEffect(() => {
    if (activeSong && activeSong.location) {
      window.electron.getMusicMetaData(activeSong.location).then((metaData) => {
            ...
            >
              <div>
                ...
                <div className={classes.volumeContainer}>
                  ...
                  >
                    <Button aria-label="Volumne control">
                      {getVolumeIcon()}
                    </Button>
                  </Tooltip>
                  <Button
